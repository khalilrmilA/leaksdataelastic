...
# Parsing
# =========================
def _parse_hash_format(line: str) -> dict | None:
    """
    Parse url#login@password format (e.g., WordPress-style exports).
    The login may be an email address or a plain username.
    """
    hash_idx = line.index("#")
    url_part = line[:hash_idx]
    rest     = line[hash_idx + 1:]

    if not url_part or not rest:
        return None
    ...
    # Try to locate a valid email inside 'rest'; what follows is the password
    m = EMAIL_REGEX.search(rest)
    if m:
        email    = m.group(0)
        password = rest[m.end():].lstrip("@")
        return {
            "url":          url_part,
            "url_domain":   url_domain,
            "email":        email.lower(),
...
def parse_leak_line(line: str) -> dict | None:
    """
    Parse credential lines in multiple formats:
      1. url:email:pass           (standard ULP)
      2. email:pass               (no URL)
      3. url:username:pass        (non-email login)
      4. https://url:login:pass   (scheme included in URL)
      5. url#login@pass           (hash separator, WordPress-style)
      6. android://hash@app/:email:pass
    ...
    # ── Hash-separator format (url#login@password) ───────────────────────
    if "#" in line:
        result = _parse_hash_format(line)
    ...
    # 'https://host/path' splits into ['https', '//host/path', ...].
    # Determine how many leading parts belong to the URL scheme.
    url_end = 2 if (len(parts) > 1 and parts[1].startswith("//")) else 1

    # Locate an email token, skipping URL path segments (they start with '//')
    # so that '@' inside android:// or https:// URLs is not mistaken for an email.
    email_idx = -1
    ...
    if url_domain and len(remaining) >= 2:
